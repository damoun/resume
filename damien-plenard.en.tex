% !TEX program = xelatex

\documentclass{resume}

\begin{document}
\pagenumbering{gobble} % suppress displaying page number

\name{Damien Plenard}

\basicInfo{
  \birthday{1990-01-29} \textperiodcentered\
  \email{damien@plenard.me} \textperiodcentered\
  \phone{+31638014933}
}

\section{\texorpdfstring{\textbf{\faicon{user}}\ Experience}{Experience}}

\datedsubsection{\textbf{Mollie}}{2022 -- present}
\role{Site Reliability Engineer}{}
Brief introduction: Support the migration to Google Cloud Platform.
\begin{itemize}
  \item Take the ownership of the On-Premises infrastructure
  \item Automate the onboarding and maintenance of the servers with Infrastructure-as-Code
\end{itemize}

\datedsubsection{\textbf{Veepee}}{2018 -- 2022}
\role{Site Reliability Engineer}{}
Brief introduction: Provide company wide monitoring solutions
\begin{itemize}
  \item Manage a company wide Elasticsearch cluster for logs
  \item Collect and aggregate monitoring metrics for long time storage with Prometheus & Thanos
  \item Create a SaaS platform for monitoring components using Nomad & Docker
  \item Automate website purchase path monitoring with Selenium
  \item Improve incident management workflow with automation
\end{itemize}

\datedsubsection{\textbf{Dalenys}}{2012 -- 2018}
\role{System Engineer}{}
Brief introduction: Maintain and develop projects related to carrier billing
\begin{itemize}
  \item Develop an internal phone numbers annuary for portability
  \item Implement mulitple carrier billing platforms
  \item Maintain the bulk & premium text message platform
  \item Assist the operation team with its own deployment solution
  \item Update Quality Assurance and Continuous Integration of various projects
\end{itemize}

\section{\texorpdfstring{\textbf{\faicon{graduation-cap}}\ Education}{Education}}

\datedsubsection{\textbf{Epitech}, Paris, France}{2009 -- 2014}
\textit{Master of Science} in Computer Science and Engineering
\datedsubsection{\textbf{Beijing Jiaotong University (BJTU)}, Beijing, China}{2012 -- 2013}
3 Terms in Software Engineering Technology

\section{\texorpdfstring{\textbf{\faicon{cogs}}\ Skills}{Skills}}

\begin{itemize}[parsep=0.5ex]
  \item Programming Languages: Python, Terraform, Ansible
  \item Platform: Debian, Docker, Prometheus, Elastic
  \item Development: Gitlab CI/CD, SQL, Selenium
\end{itemize}

\section{\texorpdfstring{\textbf{\faicon{info}}\ Miscellaneous}{Miscellaneous}}

\begin{itemize}[parsep=0.5ex]
  \item Languages: English - Professional, French - Native speaker
  \item \github[github.com/damoun]{https://github.com/damoun}
  \item \linkedin[linkedin.com/in/dplenard]{https://www.linkedin.com/in/dplenard}
\end{itemize}

\end{document}
